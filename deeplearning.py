...
# %%
model_x = timm.create_model('xception', num_classes=2, pretrained=True).to(device)
x = torch.randn(32, 3, 224, 224).to(device)
optimizer_x = optim.Adam(model_x.parameters(),lr=0.001)
model_x(x).shape
# %%
model_x=DataParallel(model_x)
# %%
...
def train_and_evaluate(model, train_loader, test_loader, validate_loader, criterion, optimizer, device, num_epochs):
    ...
    plt.subplot(1, 2, 1)
    plt.plot(range(1, num_epochs + 3), train_losses, label="Training Loss")
    plt.plot(range(1, num_epochs + 3), val_losses, label="Validation Loss")
    ...
    plt.subplot(1, 2, 2)
    plt.plot(range(1, num_epochs + 3), train_accuracies, label="Training Accuracy")
    plt.plot(range(1, num_epochs + 3), val_accuracies, label="Validation Accuracy")
    plt.xlabel("Epoch")
    plt.ylabel("Accuracy (%)")
